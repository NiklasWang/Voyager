...
int32_t FramePairQueue<T>::enqueue(typename AlgTraits<T>::TaskType &task)
{
    ...
    if (task.format == FORMAT_YUV_MONO) {
...
int32_t FramePairQueue<T>::getFrame(int32_t index,
    Format format, typename AlgTraits<T>::TaskType &frame)
{
    int32_t rc = NO_ERROR;
    int32_t size = format == FORMAT_YUV_MONO ?
...
template <typename T>
uint32_t FramePairQueue<T>::size(Format format)
{
    return format == FORMAT_YUV_MONO ?
        mMonoQ.size() : mColorQ.size();
}

template <typename T>
int32_t FramePairQueue<T>::getLastTask(Format format,
