...
template <typename T>
int32_t BufHolder<T>::add(typename AlgTraits<T>::TaskType &task, int64_t allocsize)
{
    int32_t rc = NO_ERROR;

    allocsize = allocsize == 0 ? FrameHelper::size(task) : allocsize;
    ...
    typename AlgTraits<T>::TaskType &task, int64_t allocsize)
{
    ...
    if (SUCCEED(rc))  {
        allocsize = allocsize == 0 ? size : allocsize;
    }
    ...
    if (SUCCEED(rc)) {
        if (allocsize != mBufs[index]->replace(task, allocsize)) {
            LOGE(mModule, "Failed to replace buffer %d %ll/%ll bytes.",
                index, size, allocsize);
            rc = NO_MEMORY;
        }
    }
