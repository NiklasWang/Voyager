...
int32_t PfLoggerImpl::stop(const char *name)
{
    ...
    if (SUCCEED(rc) && NOTNULL(set)) {
        set->stop.update();
        (*set)();
        LOGD(mModule, "Stop testing %s, started at %lld, " \
    ...
    usec(u)
{
    if (usec == INT64_INVALID_VALUE) {
        usec = TimeUtils::getTimeUs();
    }
}

int64_t PfLoggerImpl::Timer::Time::update()
{
    return usec = TimeUtils::getTimeUs();
}
...
void PfLoggerImpl::Timer::output()
{
    ...
    LOGI(module, " >> Dump performance record %s", name.c_str());
    for (auto &t : mSet) {
        if (NOTNULL(t)) {
            index++;
            total += (*t)();
    ...
    LOGI(module, " >> Finished display record %s, " \
        "tested %d times, average %.3f ms", name.c_str(), index,
