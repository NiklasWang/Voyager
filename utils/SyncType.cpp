...
SyncType::SyncType(SyncType::value_type type) :
    mType(type),
    mSem(NULL)
{
    if (type == SYNC_TYPE) {
...
SyncType &SyncType::operator=(const SyncType::value_type &rhs)
{
    if (*this != rhs) {
...
bool SyncType::operator== (const SyncType::value_type &rhs)
{
    return mType == rhs;
}
...
const char *SyncType::whoami(SyncType::value_type type)
{
