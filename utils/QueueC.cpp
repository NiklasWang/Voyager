...
QueueC::QueueC(release_data_fn_c data_rel_fn, void *user_data)
{
    pthread_mutex_init(&m_lock, NULL);
    clist_init(&m_head.list);
    m_size = 0;
...
bool QueueC::enqueueWithPriority(void *data)
{
    bool rc;
    q_node *node =
        (q_node *)malloc(sizeof(q_node));
...
void* QueueC::dequeue(bool bFromHead)
{
    q_node* node = NULL;
    void* data = NULL;
    struct clist *head = NULL;
...
void* QueueC::dequeue(match_fn_data_c match, void *match_data)
{
    q_node* node = NULL;
    struct clist *head = NULL;
    struct clist *pos = NULL;
...
void QueueC::flushNodes(match_fn_c match)
{
    q_node* node = NULL;
    struct clist *head = NULL;
    struct clist *pos = NULL;
...
void QueueC::flushNodes(match_fn_data_c match, void *match_data)
{
    q_node* node = NULL;
    struct clist *head = NULL;
    struct clist *pos = NULL;
