...
static inline void clist_add_tail_node(
    struct clist *item, struct clist *head)
{
...
static inline void clist_insert_before_node(
    struct clist *item, struct clist *node)
{
    item->next = node;
    item->prev = node->prev;
    item->prev->next = item;
