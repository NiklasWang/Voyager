...
MemMgmtImpl::MemMgmtImpl(std::string name, bool enablePool) :
...
void *MemMgmtImpl::malloc(size_t size, const char *file,
...
void MemMgmtImpl::free(void *data, const char *file,
    const char *func, const int32_t line)
{
    if (data == NULL) {
...
void MemMgmtImpl::dump(const char *file, const char *func, const int32_t line)
{
...
void *MemMgmtImpl::__malloc(size_t size)
{
    void *result = NULL;
    if (mEnablePool && NOTNULL(mPool)) {
...
void MemMgmtImpl::__free(void *ptr)
{
    if (mEnablePool && NOTNULL(mPool)) {
        PoolFree(mPool, ptr);
    } else {
